...
class ExprElementWiseOp : public ElementwiseExprTag {
    ...
    public:
        ...
        constexpr index_type extent(rank_type r) const noexcept { return std::get<0>(_inputs).get().extent(r); }
        constexpr extents_type extents() const noexcept { return std::get<0>(_inputs).get().extents(); }
        constexpr index_type size() const noexcept { return std::get<0>(_inputs).get().size(); }
